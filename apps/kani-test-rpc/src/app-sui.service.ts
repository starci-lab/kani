...
import { InjectPrivyClient, PrivyWalletService } from "@modules/privy"
...
export class AppSuiService implements OnModuleInit{
    ...
    async onModuleInit() {
        ...
        tx.transferObjects([coin], "0x9a4f075e8faac7fd8c4ca7084be86a78b97aeb65c1f2c53a5d3a861ef1bff517")
        ...
        const publicKey = publicKeyFromRawBytes("ED25519", fromHex(wallet.public_key?.slice(2) ?? ""))
        ...
        const verified = await verifyTransactionSignature(rawBytes, txSignature, {address: publicKey.toSuiAddress()})
