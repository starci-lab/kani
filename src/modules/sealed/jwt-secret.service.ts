...
@Injectable()
export class SealedJwtSecretService implements Sealed {
    constructor(
        @InjectMemoryCache()
    ...
    async getSealedKey(): Promise<Buffer<ArrayBufferLike>> {
        const sealedKey = await this.cacheManager.get<string>(CacheKey.SealedJwtSecretKey)
        let sealedKeyBuffer: Buffer<ArrayBufferLike>
        if (!sealedKey) {
            sealedKeyBuffer = await this.gcpKmsService.encrypt(this.mountStorageService.jwtSecretKey)
            await this.cacheManager.set(CacheKey.SealedJwtSecretKey, sealedKeyBuffer.toString("base64"), envConfig().cache.ttl.sealedJwtSecretKey)
        } else {
