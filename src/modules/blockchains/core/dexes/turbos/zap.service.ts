...
export class TurbosZapService implements IZapService {
    ...
    async computeZapAmounts(
        ...
        const { amountOut, routerId, quoteData } = await this.retryService.retry({
            maxRetries: 10,
            delay: 200,
            action: async () => {
                ...
                const minAmountOut = toScaledBN(receiveAmount, new Decimal(1).minus(slippage))
                if (amountOut.lt(minAmountOut)) {
                    throw new Error(
                        `Quote amount too low: got=${amountOut.toString()} min=${minAmountOut.toString()}`
                    )
                }
                return { amountOut, routerId, quoteData }
            },
        })
        ...
        // Price impact
        const priceImpact = new Decimal(ratio)
            .sub(actualRatio)
            .div(ratio)
