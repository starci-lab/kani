...
// we only keep the last 5000 series in the history
// the older series are discarded
export const HISTORY_SERIE_COUNT = 5000

@ObjectType({ description: "Represents a history serie" })
@Schema({ autoCreate: false })
export class HistorySerieSchema {
    @Field(() => Date, { description: "The date and time the position was closed" })
    @Prop({ type: Date, required: true })
        positionClosedAt: Date

    @Field(() => Number, { description: "The position value at close" })
...
export class HistorySchema extends AbstractSchema {
    @Prop({ type: [HistorySerieSchemaClass], required: true })
    ...
    @Prop({ type: Types.ObjectId, ref: BotSchema.name })
